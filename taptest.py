...
def tap(activity, function, delay=2, offset=0):
    ...
    :param activity: 需要点击的界面
    :param function: 需要点击的对应界面下的功能按钮
    :param delay: 点击后等待的延迟(秒),等待程序作出响应完成对应点击操作
                  按照不同功能其响应时间有变化
                  (Default value = 2)
    :param offset: 随机偏移范围值(正负像素范围),
                   使得点击位置位于目标值的设定区间内随机点取
                   (Default value = 0)
    """
    global platform
...
def screencap(fileName="screenshot.png"):
    """# 抓取截屏并传送到本地
    :param fileName: 只指定文件名,
                     保存路径由environment全局变量指定
